...
public class LiteModMain implements Tickable, ChatFilter,PostRenderListener{
			...
			if(messages != null){
				if(messages.size() > 0 || urlVersion != 0 & downloadURL != ""){					
					if(urlVersion > Integer.valueOf(this.getVersion())){
						...
						minecraft.thePlayer.addChatMessage(new TextComponentString(downloadURL));		
			...
			if(isEnableselection()){
				if(minecraft.gameSettings.keyBindUseItem.isKeyDown()){
					if((System.currentTimeMillis() - lastFirstMarkPick) >=500){
						if(minecraft.thePlayer.getActiveItemStack() != null){
							if(minecraft.thePlayer.getActiveItemStack().getItem() != null){
								if(minecraft.thePlayer.getActiveItemStack().getItem().equals(Item.getByNameOrId("wooden_shovel"))){
									RayTraceResult pos = minecraft.thePlayer.rayTrace(5,1.0F);
				...
				if((System.currentTimeMillis() - lastSecondMarkPick) >=500){
					if(minecraft.gameSettings.keyBindUseItem.isKeyDown()){
						if(minecraft.thePlayer.getActiveItemStack() != null){
							if(minecraft.thePlayer.getActiveItemStack().getItem() != null){
								if(minecraft.thePlayer.getActiveItemStack().getItem().equals(Item.getByNameOrId("wooden_axe"))){
									RayTraceResult pos = minecraft.thePlayer.rayTrace(5,1.0F);
	...
	private void sendMessage(String msg){
		...
		for(String text : messages){
			minecraft.thePlayer.addChatMessage(new TextComponentString(formateTextColor(prefix + text)));
		}		
	}
	@Override
